...
class CompAdministrationModel extends Model
{
    ...
    public function editCompAdministration($post_param){
        $admin_id=$post_param['id'];$comp_id=$post_param['comp_id'];unset($post_param['comp_id']);unset($post_param['id']);
        $i=0;
        foreach ($post_param as $key=>$val){
            $result[$i]['score']=Db::name('comp_score_log')->where(['comp_id'=>$comp_id,'key_name'=>$key])->sum('score');
            $result[$i]['key_name']=$key;
            $i+=1;
        }
